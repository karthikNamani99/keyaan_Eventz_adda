...
public class Cattering_Adapter extends RecyclerView.Adapter<Cattering_Adapter.MyViewHolder> {
    private List<Catering_List> eventList1 ;
    private Context context;

    public Cattering_Adapter(List<Catering_List> events, Context mContext) {
...
//    }
    ...
    @Override
    public Cattering_Adapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.catering_listitems_main, parent, false);

        Cattering_Adapter.MyViewHolder myViewHolder = new Cattering_Adapter.MyViewHolder(itemView);
        return myViewHolder;
    }

    @Override
    public void onBindViewHolder(Cattering_Adapter.MyViewHolder holder, int position) {
        Catering_List event = eventList1.get(position);
